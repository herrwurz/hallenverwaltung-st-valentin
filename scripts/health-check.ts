...
// Usage: HEALTHCHECK_URL=http://localhost:3000/api/health npm run health:check
...
async function triggerAlert(text: string) {
  try {
    await sendAlert({ subject: "Healthcheck alert", text });
  } catch (e) {
